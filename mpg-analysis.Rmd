...
Our goal is to analyse the links between the variables of the *mtcars* data set. More specifically, we will try to see if the miles per gallon (*pmg*) efficiency can be predicted by the transmission type (*am*), and if such a link exists, we will try to quantify it. 
...
(R markdown exported to html to keep the `sjt.lm` model comparison table, then saved to HTML)
